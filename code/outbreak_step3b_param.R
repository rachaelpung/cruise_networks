...
load('github/data/edgelist weighted/edgeListTemporal_sail_1.RData')
...
# load('github/data/foi/foi_scale_set_3_sail_1.RData') # mask efficacy reduced from 80% to 70%
...
# parameters for temporal network transmission
paramTemporal = copy(param)
paramTemporal[,initial.cases.onset:=sample(distOnset.3$DAY_ONSET, num.initial.cases, prob = distOnset.3$PDF, replace = T), by = .(param.set)]
paramTemporal[,cap_max_days:=3]

paramTemporal=paramTemporal[rep(seq_len(nrow(paramTemporal)), times=3*2*5),]
paramTemporal[,intervention:=rep(c("nothing", "isolation", "isolation"), each=1000, len = .N)]
paramTemporal[,testing.regime:=rep(c("none", "PCR_once", "Ag_twice"), each=1000, len = .N)]
paramTemporal[,vac.coverage:=rep(seq(0,1,0.25), each =3*1000, len = .N)]
paramTemporal[,mask:=rep(c(0,1), each = .N/2)]
paramTemporal[,net.type:='temporal']

paramTemporal[,scenario:=paste(intervention,testing.regime,'vac',vac.coverage,'mask',mask, sep = '_')]
paramTemporal[,.N,by=.(scenario)]
